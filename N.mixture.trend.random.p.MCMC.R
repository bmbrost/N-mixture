N.mixture.trend.random.p.MCMC <- function(Y,W,priors,tune,start,n.mcmc=1000){  
  
  ###
  ### Brian M. Brost (25APR2015)
  ###
  ### N-mixture model with temporal trend and random effect on p
  ###
  ...
  ### logit(p[i,t])~W[i,,t]%*%alpha+eps[i,t]
  ...
  ### priors=parameters of prior distributions for lambda, alpha, theta, and zeta
  ### tune=tuning parameters for N, alpha, theta, zeta, and eps
  ### start=starting values for N,lambda, alpha, theta, zeta, and eps
  ###
...
#     browser()
  ...
  keep <- list(N=0,alpha=0,theta=0,zeta=0,eps=0)
  ...
  eps.save <- matrix(NA,n.mcmc,qY*T)
  ...
  zeta <- start$zeta
  eps <- start$eps
  p <- matrix(expit(W%*%alpha+eps),T,qY,byrow=TRUE)
  ...
  for(k in 1:n.mcmc){
    ...
    p.star <- matrix(expit(W%*%alpha.star+eps),T,qY,byrow=TRUE)
    ...
    eps.star <- eps
    eps.star[na.idx] <- rnorm(n,eps[na.idx],tune$eps)
  ...
  list(alpha=alpha.save,N=N.save,lambda=lambda.save,theta=theta.save,zeta=zeta.save,eps=eps.save,
